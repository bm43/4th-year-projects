...
if __name__ == "__main__":
    ...
    #environment = Environment(display=True, magnification=500) # tutorial
    environment = Environment(magnification=500) # part 2
    # Create an agent
    agent = Agent()
    ...
    while time.time() < end_time:
        ...
        action = agent.get_next_action(state) # returns continuous_action like [0,1]
        # Get the next state and the distance to the goal
        next_state, distance_to_goal = environment.step(state, action) # return next_state, distance_to_goal
        # Return this to the agent
        agent.set_next_state_and_distance(next_state, distance_to_goal) # this executes train_q_network function
        # Set what the new state is
        state = next_state

        # Optionally, show the environment
        if display_on:
            #environment.draw(state) # tutorial
            environment.show(state) # part 2
    agent.dqn.show_loss()
    ...
    # save model
    torch.save(agent.dqn.q_network.state_dict(),'C:/Users/SamSung/Desktop/uni/y4/RL/DQN_tutorial/Coursework/tutorial_env_answer.pth')
    print('model saved!')
